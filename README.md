...
`plass-ui/tailwind.css` registers all 55 components with Tailwind, because Tailwind scans files rather than imports — nothing in a build connects `import { PlButton }` to the classes `PlSelect.js` spells out. A project that uses a handful of components can register the handful instead:
...
Every component also has an entry point of its own, for a build that cannot tree-shake a barrel — or for a server render, where the barrel loads all 55 components and their dependencies before the first one is used:
...
`PlAlert` · `PlDrawer` · `PlModal` · `PlOverlay` · `PlPopover` · `PlSkeleton` · `PlToast` · `PlTooltip`
