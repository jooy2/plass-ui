...
그 wrapper 안쪽 네 부분에 닿는 것이 `classNames`입니다: `label`, `control`(트랙), `description`, `error`.
...
차이는 **시간**에 있습니다. checkbox는 form과 함께 제출되는 값이고, switch는 움직이는 순간 적용됩니다. 아래에 Save 버튼이 있다면 그것은 checkbox였어야 합니다.
...
- Base UI가 `aria-checked`가 붙은 `role="switch"` 컨트롤을 렌더링하고, `name`을 주면 네이티브 form 제출에 포함되는 hidden input도 함께 렌더링합니다.
