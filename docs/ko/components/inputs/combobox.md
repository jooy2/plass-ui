...
고른 값들이 field 안의 [`PlChip`](../display/chip)이 되고, 입력은 그 뒤로도 계속 필터링합니다.
...
- 목록은 선택된 행에, `multiple`에서는 목록에서 가장 위에 있는 선택된 행에 불이 들어온 채로 열리고, 방금 취한 행은 계속 켜져 있습니다. 선택된 것이 없으면 화살표 키로 열 때는 첫 행에 불이 들어오고, 눌러서 열 때는 어느 행에도 들어오지 않습니다.
