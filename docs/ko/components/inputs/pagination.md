...
현재 페이지는 `aria-current`가 붙은 링크로 남고, 범위 끝의 이동 버튼은 주소 없이 `aria-disabled`가 붙은 `<a>`로 남습니다. 누른 컨트롤이 다른 요소로 바뀌면 독자의 포커스도 함께 사라지고, 주소가 없으니 크롤러가 따라갈 곳도 없습니다. `getPageHref`가 없으면 끝에 닿은 이동 버튼은 사용할 수 없다고 알리면서 탭 순서에는 그대로 남는 버튼입니다.
...
- 모든 버튼에 접근 가능한 이름이 있습니다 (`Page 4`, `Next page`). 이 이름들과 live 문장은 `PlassProvider`가 정한 라벨 묶음에서 오고, 페이저 하나만 다르게 하려면 prop을 주면 됩니다. 여기 있는 문자열은 화면에 그려지지 않습니다.
...
- 모든 버튼에 자기 이름이 있습니다. "Page 4", "Next page". 이름은 테마의 라벨 묶음에서 오고, 줄 하나만 다르게 하려면 파라미터를 주면 됩니다. 여기 있는 문자열은 화면에 그려지지 않습니다.
