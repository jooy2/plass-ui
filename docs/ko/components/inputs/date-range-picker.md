...
반쪽짜리 구간은 실제로 존재하는 상태입니다(첫 클릭과 둘째 클릭 사이에 picker가 들고 있는 것이 그것입니다). 그래서 `onValueChange`는 첫 클릭 뒤에 `{ start, end: null }`을, 둘째 뒤에 완성된 구간을 보고합니다. controlled 호출자도 이 반쪽 구간을 그대로 받으므로, 둘째 클릭 전까지는 `end` 없이 `start`만 있는 값을 봅니다. 컴포넌트 안에 남는 것은 대기 중인 anchor이고, 그것이 첫 클릭과 둘째 클릭을 가립니다. 그래서 호출자가 직접 넣었거나 popup이 닫히며 남은, `start`만 있는 `value`는 그 선택을 마치는 대신 새 선택을 시작합니다.
