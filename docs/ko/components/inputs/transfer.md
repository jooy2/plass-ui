...
열두 개쯤 아래라면 그 둘 중 하나가 더 작은 컴포넌트입니다. 이것은 독자에게 목록 둘과 화살표 한 쌍을 치르게 하는데, 그 값을 하는 것은 답 자체를 되읽을 값어치가 있을 때입니다.
...
- 각 제목의 체크도 체크박스이고, `selectAllLabel` 뒤에 자기 목록의 제목을 붙인 "Select all Available"이 이름입니다. 그래서 두 체크를 귀로도 구별할 수 있습니다. 목록의 일부만 체크된 상태에서는 `indeterminate`를 보고합니다.
...
- 옮기고 나면 포커스는 행이 도착한 목록의 첫 행으로 갑니다. 누른 화살표는 이 이동으로 disabled가 되므로, 그대로 두면 포커스가 컴포넌트 밖으로 떨어집니다. 제어되는 쌍의 주인이 행을 받지 않으면 포커스는 그 목록 자체로 갑니다. 목록은 제목을 이름으로 가진 그룹입니다.
- 몇 개를 옮겼는지는 polite 우선순위로 한 번 안내되고, 문구는 라벨 묶음의 `transferMoved`에서 옵니다. 쌍 하나만 다르게 하려면 `movedLabel`을 주면 됩니다. 도착한 행이 없으면 아무것도 안내하지 않습니다.
