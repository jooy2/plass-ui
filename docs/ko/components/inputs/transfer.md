...
- 옮기고 나면 포커스는 행이 도착한 목록의 첫 행으로 가고, 그 목록은 그 행까지 스크롤됩니다. 누른 화살표는 이 이동으로 disabled가 되므로, 그대로 두면 포커스가 컴포넌트 밖으로 떨어집니다. 제어되는 쌍의 주인이 행을 받지 않으면 포커스는 그 목록 자체로 갑니다. 목록은 제목을 이름으로 가진 그룹입니다.
