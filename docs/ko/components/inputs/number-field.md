...
옵션 객체 하나가 아니라 함수 둘입니다. `format`은 정착한 값을 쓰고 `parse`는 입력된 글자를 되읽습니다. Dart SDK에는 `Intl.NumberFormat`이 없고 이 패키지에는 의존성이 없으니, locale을 아는 field는 앱이 자기 formatter로 만드는 것입니다. 어차피 다른 화면에도 필요해서 이미 갖고 있을 그 formatter를 씁니다.
...
- 두 스테퍼에는 이미 이름이 있습니다. `incrementLabel`과 `decrementLabel`이 그것을 바꿉니다. 각각은 숫자 다음의 자기 focus stop이 있습니다.
...
- `allowWheelScrub`은 기본적으로 꺼져 있고, 켜도 field가 focus가 붙은 **동시에** 포인터가 그 위에 있어야 합니다. 포인터 아래에서 스크롤되는 페이지와 값이 바뀌는 field는 같은 동작이고, 의도된 것은 둘 중 하나뿐입니다.
