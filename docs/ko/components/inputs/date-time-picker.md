...
그날을 고르면 이미 맞춰 둔 시각을 유지하되, 경계 밖이면 경계 안으로 옮깁니다. 08:00이 맞춰져 있거나 아직 시각이 없으면 27일을 골랐을 때 09:30이 됩니다. 경계가 분 사이에 있으면 안쪽으로 올리거나 내리므로, 09:30:15는 09:31이 되고 `showSeconds`가 켜져 있으면 다음 초가 됩니다.
