...
- 모든 선택지는 그것이 나타내는 점수로 이름이 붙습니다 (`3 out of 5`). 문장은 라벨 묶음의 `ratingValue`에서 오고, 소수점이 있는 점수는 그 언어가 숫자를 쓰는 방식을 따라 독일어에서는 `2,5 von 5`로 읽힙니다. rating 하나만 다르게 하려면 `valueLabel`을 주면 됩니다. 여기서 화면에 그려지는 것은 없습니다.
