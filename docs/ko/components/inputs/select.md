...
- trigger는 보여 줄 수 있는 가장 긴 라벨의 너비로 벌어져 있습니다. 짧은 옵션을 골랐다고 방금 고른 포인터 아래에서 필드가 줄어들지 않습니다. 이 샘플들은 `aria-hidden`이고 생성 콘텐츠로 그려지므로, 읽히지도 않고 페이지 내 검색에 걸리지도 않습니다. 요소로 된 라벨은 텍스트만으로 너비를 잽니다. 그 안의 그림은 너비를 재려고 불러오지 않으며, 그 옵션을 고르면 trigger가 그림 너비만큼 넓어질 수 있습니다.
...
- trigger는 필드의 `label`을 이름으로 하는 버튼으로 읽히고, 골라진 것은 값으로, 목록이 열려 있는지도 함께 읽힙니다. `label`이 `Text`면 그 글자가 알아서 이름이 되고, 옆에서 한 번 더 읽히지 않습니다. 다른 위젯으로 만든 label에는 `semanticLabel`이 필요합니다. 각 행은 서로 배타적인 묶음의 하나로, 골라졌는지 아닌지와 함께 읽힙니다.
...
- 하이라이트는 행마다의 hover 상태가 아니라 숫자 하나입니다. 포인터와 방향키가 같은 행을 비추게 하는 것이 그것입니다. 포인터는 행 위로 움직여야 그 행을 비추므로, 방향키가 목록을 스크롤해 멈춰 있는 포인터 밑으로 다른 행이 와도 하이라이트는 방향키가 고른 행에 남습니다.
- 고를 수 없는 행도 목록에 남고, 사용할 수 없다고 읽힙니다. 고를 수 없다고 사라지는 옵션은 읽는 사람이 계속 찾게 되는 옵션입니다.
- trigger는 말할 수 있는 가장 긴 라벨의 너비로 벌어져 있습니다. 그 샘플들은 배치되되 그려지지 않고 semantics에서도 제외되므로, 읽히는 것이 늘지 않습니다. 평범한 `Text`가 아닌 라벨은 그 안의 글자만으로 너비를 잽니다. 그 안의 그림은 너비를 재려고 만들지 않으며, 그 옵션을 고르면 trigger가 그림 너비만큼 넓어질 수 있습니다.
