...
<p class="plass-lede">한 글자짜리 칸이 늘어선 줄입니다. PIN이나 문자로 받은 인증 코드, 초대 키에 씁니다. 칸이 몇 개든 그 뒤에는 값 하나가 있고, 붙여넣기와 백스페이스, 휴대폰의 자동 완성이 모두 독자가 기대하는 대로 동작합니다.</p>
...
- 칸마다 라벨 뒤에 줄 안의 위치를 붙인 이름이 있습니다. "Verification code Character 2 of 6"처럼 읽힙니다. Base UI는 칸마다 라벨만 이름으로 주기 때문에, 그대로 두면 같은 이름을 여섯 번 읽고 캐럿이 어느 칸에 있는지는 말하지 않습니다. 위치 문구는 라벨 묶음의 `otpSlot`에서 오고, 필드 하나만 다르게 하려면 `slotLabel`을 주면 됩니다.
