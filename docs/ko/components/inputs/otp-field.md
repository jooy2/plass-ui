...
칸은 컨트롤 사다리가 아니라 자기 사다리를 씁니다. 체크박스의 틱과 같은 이유입니다 — 칸은 컨트롤들 사이의 컨트롤이 아니라 홀로 선 글자 하나이고, `md` `PlButton` 높이의 `md` 칸은 책상 건너에서 코드를 읽기에 너무 작습니다. 모든 단계가 너비보다 높이가 크고, 그것이 이 줄을 작은 field의 행렬이 아니라 글자 하나씩의 자리로 읽히게 합니다.
