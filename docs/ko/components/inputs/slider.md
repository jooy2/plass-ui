...
끌어서 옮긴 것이 아닌 값 — 화살표 키, rail을 누른 것, 바깥에서 바꾼 값 — 으로는 thumb이 **이동합니다**. 나머지 전부가 움직이는 것과 같은 duration이고, 뒤의 run도 같은 속도로 찹니다. 손가락 아래에서는 전혀 이동하지 않습니다. 포인터를 향해 서서히 따라가는 thumb은 포인터보다 뒤처지는 thumb이고, 그것은 부드러운 모션이 아니라 느린 컨트롤로 읽힙니다. 라이브러리에서 위치에 애니메이션을 주는 곳은 여기 하나뿐이며, [transform 금지 규칙](../../design/design-language#컨트롤은-움직이지-않습니다)을 어기지 않습니다. 제자리에서 밀려나는 것은 없고, 움직이는 것이 곧 값이기 때문입니다.
