...
체크 표시는 박스가 채워지는 프레임에 통째로 나타나지 않고 **그려지며**, 박스를 비우면 다시 지워집니다. 한 번에 도착하는 표식은 클릭이 만든 것이 아니라 갈아 끼운 것으로 읽힙니다. 이를 위해 확대하는 것은 없습니다. 획을 자기 길이만큼의 점선으로 만들고 그 점선을 움직이므로, 체크 표시의 어느 부분도 최종 위치가 아닌 곳에 놓이지 않습니다. [모션](../../design/design-language#표식은-켜지는-것이-아니라-그려집니다)을 보세요.
