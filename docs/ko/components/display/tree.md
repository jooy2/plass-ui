...
닫힌 가지는 **아예 만들어지지 않습니다.** 행은 가지가 열려 있는 동안, 그리고 접힘이 움직일 행이 있도록 닫히는 동안에만 만들어집니다. 그래서 닫힌 폴더가 사백 개인 트리는 폴더 행 사백 개만큼만 비용이 들고, 그 아래 행에는 비용이 들지 않습니다. 애초에 보내기에 너무 큰 트리라면 위에서처럼 폴더에 빈 배열을 주고 열릴 때 채우면 됩니다.

::: fw react

focus를 옮기면 트리 전체가 아니라 떠난 행과 도착한 행, 그리고 두 행이 든 가지만 다시 그립니다.
