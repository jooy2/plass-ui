...
- **무엇보다 먼저 `getRowKey`.** 표가 기억하는 모든 것은 _key로_ 기억됩니다. 선택도, 범위의 기준점도, React와 Flutter가 행을 대조하는 정체성도. 없으면 행의 key는 `rows` 안의 위치입니다. 정렬하고 검색하고 페이지를 넘겨도 그대로지만, `rows`가 바뀌면 달라집니다. 맨 위에 행이 하나 들어오면 그 아래 행의 key가 모두 바뀌고, 그 전에 해 둔 선택은 다른 행으로 옮겨 갑니다.
