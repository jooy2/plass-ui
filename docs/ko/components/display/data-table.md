...
- `maxHeight`보다, 또는 담긴 상자보다 높은 격자는 시트 안에서 스크롤됩니다. 그동안 격자는 Tab 정지점이 되므로, 정렬할 제목도 체크박스 열도 없는 표에서도 화살표 키와 <kbd>Page Up</kbd>, <kbd>Page Down</kbd>, <kbd>Home</kbd>, <kbd>End</kbd>로 가장자리 너머의 행에 닿습니다. 정지점의 이름은 `semanticLabel`입니다. caption은 문자열이 아니라 위젯이라 이름이 될 수 없고, 대신 정지점 바로 앞에서 읽힙니다.
