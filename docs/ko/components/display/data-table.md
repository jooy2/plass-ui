...
- 시트보다 넓거나 `maxHeight`보다 높은 격자는 시트 안에서 스크롤됩니다. 그동안 격자가 스크롤되는 상자는 Tab 정지점이 되므로, 정렬할 제목도 체크박스 열도 없는 표에서도 화살표 키로 가장자리 너머의 열에 닿습니다. 이 상자는 `caption`을 이름으로 단 그룹이기도 합니다. caption이 없는 표는 `label`로 정지점에 이름을 붙이고, 둘 다 없으면 정지점에 이름이 없습니다.
- `caption`은 시트 위에 그려지고 `aria-hidden`이 붙으며, 표 안의 진짜 `<caption>`이 같은 말을 나릅니다. 스크롤되어 사라지는 caption은 표의 접근 가능한 이름을 함께 데려갑니다. caption이 없으면 `label`이 `aria-label`로 표에 이름을 붙입니다.
...
- `semanticLabel`이 없으면 `Text`인 `caption`이 표에 이름을 붙이고, 그 문구는 이름으로 한 번만 읽힙니다. 다른 위젯으로 짠 caption은 격자 위의 한 줄로 읽힙니다.
- `maxHeight`보다, 또는 담긴 상자보다 높은 격자는 시트 안에서 스크롤됩니다. 그동안 격자는 Tab 정지점이 되므로, 정렬할 제목도 체크박스 열도 없는 표에서도 화살표 키와 <kbd>Page Up</kbd>, <kbd>Page Down</kbd>, <kbd>Home</kbd>, <kbd>End</kbd>로 가장자리 너머의 행에 닿습니다. 정지점은 표의 이름으로 읽히는데, 그 이름은 `semanticLabel`이고 없으면 `Text`인 caption의 문구입니다.
...
- **무엇보다 먼저 `getRowKey`.** 표가 기억하는 모든 것은 _key로_ 기억됩니다. 선택도, 범위의 기준점도, React와 Flutter가 행을 대조하는 정체성도. 없으면 행의 key는 `rows` 안의 위치입니다. 정렬하고 검색하고 페이지를 넘겨도 그대로지만, `rows`가 바뀌면 달라집니다. 맨 위에 행이 하나 들어오면 그 아래 행의 key가 모두 바뀌고, 그 전에 해 둔 선택은 다른 행으로 옮겨 갑니다. `manual` 페이징에서는 그 위치를 첫 페이지의 첫 행부터 셉니다. 그래서 한 페이지에서 고른 행이 다음 페이지의 같은 자리 행에 켜지지는 않지만, 서버가 행 하나를 더해 같은 페이지를 다시 보내면 그 아래 key는 역시 모두 바뀝니다.
- **콜백이 받는 `index`는 모두 `rows` 안의 위치입니다.** 화면에서 몇 번째인지가 아닙니다. `getRowKey` · `render` · `onRowClick` · `isRowSelectable`(Flutter에서는 `rowKey` · `cell` · `onRowPressed` · `isRowSelectable`)은 정렬이나 검색이나 페이지가 행을 어디로 옮기든 그 행에 같은 숫자를 받습니다. `manual` 페이징에서는 `rows`가 서버가 보낸 한 페이지이므로 그 페이지의 맨 위부터 셉니다.
