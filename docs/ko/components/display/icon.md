...
상자는 `inline-flex`이고 글리프는 그 상자를 채우도록 지시받으며, `font-size`도 같은 길이로 설정됩니다. 그래서 자체 `width`가 붙은 `<svg>`, `em`으로 그려진 `<svg>`, 문자 하나, `<img>`가 모두 같은 크기로 나옵니다.
...
- `label`이 있으면 그 이름이 붙은 `role="img"`가 됩니다. 글리프가 혼자서 뜻을 나르고 있을 때만 넘기세요.
...
- `label`이 있으면 그 이름이 붙은 이미지 노드가 됩니다. 글리프가 혼자서 뜻을 나르고 있을 때만 넘기세요.
