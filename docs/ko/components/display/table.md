...
격자 아래의 시트이고, 다른 모든 컨테이너와 같은 세 가지 재질을 쓰며 색이 들어가지 않습니다. 헤더 띠는 색을 입는 대신 유리 사다리에서 한 단계 위로 올라갑니다 — 열 이름 뒤에 색 띠를 두르는 것은 데이터를 chrome처럼 보이게 만드는 가장 빠른 방법입니다.
...
`striped`는 한 행 걸러 하나씩 옅게 칠합니다. 눈이 가로로 길게 따라가야 하는 넓은 표에서는 도움이 되고, 좁은 표에서는 소음입니다. `hoverable`은 포인터 아래의 행을 색 계열의 soft 틴트로 밝힙니다.
...
- 셀 여백과 정렬, 행 사이의 선은 inline style로 씁니다. 호스트 스타일시트가 `td`와 `th`를 태그 이름으로, utility class가 이길 수 없는 specificity로 스타일링하기 때문이고, 라이브러리에서 이 문제를 우회해야 하는 컴포넌트는 이것 하나뿐입니다.
