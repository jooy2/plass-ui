...
<p class="plass-lede">작고 촘촘한 토큰입니다. 태그와 필터, 상태, 목록에서 집어낸 개체 하나에 씁니다. 개수를 달 수도, 눌릴 수도, 지워질 수도, 셋 다일 수도 있습니다.</p>
...
`count`는 자기 판 위에 그려집니다. 그래서 "Errors 12"가 두 단어가 아니라 개수가 붙은 하나의 토큰으로 읽힙니다.
...
- ×는 라벨 크기로 그려지지만, × 둘레의 24px 정사각형 안 어디를 눌러도 ×가 눌립니다. WCAG 2.5.8이 요구하는 최소 target 크기입니다. chip의 크기는 그대로입니다. 정사각형이 라벨에 걸치는 곳에서는 정사각형 안쪽만 ×가 받고, 나머지는 라벨이 받습니다.
