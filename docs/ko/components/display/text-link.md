...
**돌려준 요소에 `href`가 있으면 그쪽이 이깁니다.** 라우터의 `Link`는 주소를 그대로 넘기는 것이 아니라 *해석*합니다 — 로케일 접두어, base path, 타입 있는 경로 — 그리고 그 결과를 앵커에 답니다. 그래서 요소가 자기 `href`를 갖고 있으면 건드리지 않고, `href`가 없는 요소에만 위의 `href`를 얹습니다. 반대로 만들면 앱의 모든 로케일 링크가 조용히 접두어를 잃습니다. 앵커에는 날 문자열이 남고, 크롤러도 가운데 클릭도 복사한 주소도 전부 엉뚱한 페이지를 가리킵니다.
