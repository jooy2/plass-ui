...
`rel`은 덮어쓰지 않고 합칩니다. 직접 `rel`을 쓰는 흔한 이유는 `nofollow`나 `sponsored`인데 그것은 SEO 결정이고, 단순한 덮어쓰기였다면 새 탭으로 열리는 링크에서 보호 장치가 조용히 사라졌을 것입니다. 직접 넘긴 `target`에도 같은 `rel`이 붙고, `target="_blank"`에는 스크린리더용 한 마디도 붙습니다. 화살표는 여전히 `newTab`과 `icon`을 따릅니다.
...
`true`는 `newTab`일 때 상자를 빠져나가는 화살표를, 아니면 체인을 그립니다. `false`는 아무것도 그리지 않고, 노드를 주면 그것이 글리프를 대신합니다. 생략하면 `newTab`을 따릅니다. 창을 가져가는 링크는 그렇다고 알려야 하고, 조용한 쪽은 호출하는 쪽이 요청해야 합니다.

:::

::: fw flutter

`showIcon`이 표시를 그릴지를, `icon`이 무엇을 그릴지를 정합니다. 생략하면 `showIcon`은 `external`을 따릅니다. 독자를 앱 밖으로 데려가는 링크는 그렇다고 알려야 하고, 조용한 쪽은 호출하는 쪽이 요청해야 합니다. `bool`이 아니라 `bool?`인 이유가 그것입니다.
...
밑줄과 표시, focus ring을 그대로 둔 채 라우터의 `Link`를 씁니다.

**돌려준 요소에 `href`가 있으면 그쪽이 이깁니다.** 라우터의 `Link`는 주소를 그대로 넘기지 않고 로케일 접두어나 base path, 타입 있는 경로를 적용해 *해석*한 뒤 앵커에 답니다. 그래서 요소가 자기 `href`를 가지고 있으면 건드리지 않고, `href`가 없는 요소에만 위의 `href`를 얹습니다. 반대로 만들면 앱의 모든 로케일 링크가 접두어를 잃습니다. 앵커에는 날 문자열이 남고, 크롤러도 가운데 클릭도 복사한 주소도 엉뚱한 페이지를 가리킵니다.
...
- `newTab`은 그려지기만 하는 것이 아니라 읽힙니다. 화살표는 볼 수 있는 사람에게, 화면에 보이지 않는 문구는 나머지 모두에게 새 탭이라고 알립니다.
- 주된 신호는 밑줄이고, 색이 유일한 신호였던 적은 없습니다. `underline="none"`은 주변이 이미 무엇인지 알려 주는 링크를 위한 것입니다.
...
- `external`은 그려지기만 하는 것이 아니라 읽힙니다. 화살표는 볼 수 있는 사람에게, 힌트는 나머지 모두에게 앱을 떠난다고 알립니다.
- 주된 신호는 밑줄이고, 색이 유일한 신호였던 적은 없습니다. `PlTextLinkUnderline.none`은 주변이 이미 무엇인지 알려 주는 링크를 위한 것입니다.
