...
`bare`는 **높이**를 정하고 너비를 따라오게 합니다. 워드마크에 필요한 것이 그것이고, 정사각형이 망가뜨리는 것도 그것입니다. 판은 아트워크를 채우지 않고 타일의 70%쯤으로 넣습니다. 그래야 글리프가 앱 아이콘이 늘 가지는 여백을 둡니다.
...
- 이름은 heading이 아니라 `<span>`입니다. 로고는 제품의 이름을 나타내고, 페이지의 heading은 페이지의 이름을 나타냅니다.
