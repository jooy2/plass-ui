...
`os`의 기본값은 `auto`이고, 브라우저에 묻습니다. 플랫폼을 명시하는 것은 페이지가 그래야 할 때뿐입니다 — Windows 빌드를 설명하는 지원 문서, 둘을 비교하는 표.
...
캡은 컨트롤 사다리에서 한 단계 내려옵니다 — `size="md"`는 40px이 아니라 32px 캡을 그립니다. 캡은 문장 속 토큰이지, 줄이 기준선을 맞추는 컨트롤이 아닙니다.
