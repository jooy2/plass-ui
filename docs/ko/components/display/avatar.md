...
사진은 파일 크기가 아니라 아바타 크기로 디코드됩니다. 넘겨준 `ResizeImage`는 그대로 씁니다.
...
- avatar는 `semanticLabel`을 받고, 없으면 `name`, 그것도 없으면 아무 이름도 갖지 않습니다. 추측하는 대신 비워 둡니다. 행 안에서 사람 이름 옆에 놓인 avatar는 장식이고, 이름을 두 번 읽는 것은 한 번만 알리는 것보다 나쁩니다.
