...
타일은 주어진 순서대로 목록 하나에 들어 있고, 각 타일을 제 lane에 그리는 일은 CSS Grid가 맡습니다. 그래서 Tab 순서와 스크린 리더도 그 순서를 따르고, 열 개수가 바뀌어 다른 lane으로 옮겨 가는 타일도 사진을 그대로 유지합니다. 다른 lane에 있는 타일의 끝을 지나 내려가는 사진은 그런 경계마다 gap 하나만큼 길게 그려지고, `caption="below"`라면 그 타일의 캡션만큼 더 길어집니다. 늘어난 높이를 사진이 어떻게 채울지는 `fit`이 정합니다.

:::

::: fw flutter

타일은 주어진 순서대로 목록 하나에 들어 있고, 각 타일은 나뉘어 들어간 lane을 따라 그려집니다. Tab 순서와 스크린 리더는 한 lane을 끝까지 내려간 뒤 다음 lane으로 넘어가지 않고 그 순서를 따르며, 열 개수가 바뀌어 다른 lane으로 옮겨 가는 타일도 사진과 focus를 그대로 유지합니다.
...
타일은 **화면 하나 거리 안에 들어와야** 사진을 요청합니다. 브라우저가 React 빌드에 해 주는 것과 같은 거래입니다. Flutter는 사진을 트리에 올리는 순간 디코딩하므로, 그러지 않으면 예순 장짜리 판이 한 장도 화면에 없는 채로 예순 번을 요청하게 됩니다.

읽는 것은 위쪽에서 가장 가까운 `Scrollable`입니다. 갤러리 자신에게는 스크롤이 없기 때문입니다. 내용 크기로 자라고 스크롤은 앱이 합니다. 위에 스크롤 뷰가 없는 갤러리는 이미 다 보이는 것이므로 예전처럼 모든 타일을 한 번에 짓습니다. 차례가 오기 전까지 타일은 사진이 로딩 중에 그리는 것과 같은 대역을 같은 박스로 들고 있어서, 사진이 도착해도 아무것도 움직이지 않습니다. 한 번 요청한 사진은 그대로 두므로 되돌아 스크롤해도 다시 시작하지 않습니다.
...
Tab 순서와 스크린 리더는 사진이 주어진 순서를 따릅니다. 그래서 앞쪽 구멍을 채운 타일도 목록에서 자기보다 앞에 있는 타일을 모두 지난 뒤에 닿습니다.
...
- 이름이 붙은 진짜 `role="list"`이고, 사진 하나당 `role="listitem"` 하나입니다.
- 타일은 눌렀을 때 무슨 일이 일어날 때만 button입니다. 이름은 **사진 자신의 말에 세트 안의 위치를 더한 것**으로, "A harbour at dusk — 1 of 6"과 같은 형태입니다. 그래서 썸네일 벽을 tab으로 지나가는 사람은 전체 몇 중 몇 번째에 있는지 듣습니다.
- 누를 수 있는 타일이 캡션을 그리면 그 `title`과 `description`이 <Fw react="aria-describedby" flutter="hint" code />로 button을 설명하고, 이름은 그대로입니다. `hover` 캡션은 포인터가 보여 주기 전에도 읽히고, 캡션이 `none`인 타일에는 읽을 것이 없습니다.
- 위치를 말하는 부분은 라벨 묶음의 `galleryItem`에서 오므로 번역된 페이지에서는 그 언어로 읽힙니다. 갤러리 하나만 다르게 하려면 `itemLabel`을 주면 됩니다. 언어마다 어순이 다르기 때문에 둘 다 슬롯이 든 문자열이 아니라 콜백입니다.
