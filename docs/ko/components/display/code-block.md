...
팔레트는 `auto`를 빼면 페이지의 명암과 무관합니다. `auto`는 페이지가 아니라 블록 위에서 가장 가까운 테마를 따르므로, 어두운 페이지 안의 밝은 패널에 놓인 블록은 밝게 그려집니다. 기본은 `dark`이고, 이것만은 취향이 아닙니다. 코드는 터미널 이래로 어두운 바탕에서 읽혀 왔고, 페이지를 따라 하얘지는 블록은 그 페이지에서 색을 코드가 아닌 다른 것이 정한 유일한 요소가 됩니다.
...
- 코드는 이름이 붙은 **focus 가능한 영역**입니다. 이름은 `title`이고, 없으면 language, 그것도 없으면 `codeLabel`, 그마저 없으면 코드를 뜻하는 낱말입니다. 스크롤되는 영역은 끌 포인터가 없는 키보드로도 닿을 수 있어야 하고, focus 가능한 영역에는 이름이 있어야 합니다.
...
- 코드는 따로 떨어진 semantics 노드이고, 이름은 language, 없으면 `codeLabel`, 그것도 없으면 라벨 팩에서 코드를 뜻하는 낱말입니다. focus가 멈추는 자리가 아니고 자체 전체 선택도 없습니다. 여기서 `title`은 문자열이 아니라 위젯이라서 이름이 되지 않습니다.
...
- 복사 버튼은 자기 라벨을 바꿉니다. 그래서 클립보드가 답하고 나면 블록이 `copiedLabel`이나 `copyFailedLabel`을 스크린 리더에 알립니다.
- raw 토글은 toggled 상태를 답니다.
