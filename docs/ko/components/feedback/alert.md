...
<p class="plass-lede">일어난 일에 대한 메시지를, 그 일이 벌어진 페이지 안에 놓습니다. 한 줄짜리, 글리프가 붙은 한 줄짜리, 헤드라인과 그 아래 상세까지 세 가지 모양이 있는데, 서로 다른 자리가 채워진 하나의 컴포넌트입니다.</p>
...
기본값은 `primary`가 아니라 `info`입니다. `primary`가 틀리는 유일한 자리입니다. alert는 알림이고, 팔레트에는 이미 그것을 가리키는 단어가 있습니다.

각 계열은 자기 색만이 아니라 자기 모양도 그립니다. 빨간색으로만 "잘못됐다"고 알리는 alert는 일부 독자에게만 닿는 alert입니다.
...
`action`은 메시지가 옆에서 줄바꿈되는 동안에도 첫 줄에 남습니다. 본문 뒤에 이어 붙이는 대신 자기 자리가 붙은 이유입니다.
...
- `action`과 닫기 버튼은 각자 tab stop이 붙은 진짜 버튼입니다. action에는 접근 가능한 이름을 주세요. 닫기 버튼은 이미 있습니다.
