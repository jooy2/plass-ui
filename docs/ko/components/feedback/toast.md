...
- Tab으로 토스트와 그 ×에 차례로 닿고, **F6**은 페이지 어디에서든 focus를 스택으로 곧장 옮깁니다. focus나 포인터가 스택 안에 들어오기 전까지 ×는 접근성 트리에서 숨겨지므로, 페이지를 훑는 스크린리더가 이미 사라졌을지도 모르는 메시지의 버튼을 만나지 않습니다.
...
- `priority`와 상관없이 모든 토스트가 live region입니다. 그래서 읽는 사람이 거기로 옮겨 가지 않아도 스크린 리더가 토스트를 읽어 줍니다. Flutter의 live region에는 정중함 단계가 polite 하나뿐이라, `high`도 React 빌드처럼 읽던 것을 끊지는 못합니다. 두 priority 모두 읽는 사람이 잠시 멈출 때 읽힙니다.
- 포인터가 스택 위에 머무르면 시계가 멈춥니다. 토스트 위에 머무른 포인터는 그것을 읽고 있는 독자이기 때문입니다. 토스트를 손가락으로 누르고 있을 때, 키보드 focus가 액션이나 × 위에 있을 때, 앱이 백그라운드로 가거나 창의 focus를 잃었을 때도 멈춥니다. 이 모두가 풀리면 멈춘 지점부터가 아니라 처음부터 다시 셉니다.
...
| `priority: 'high' \| 'low'` | 둘 다 polite live region | Flutter의 live region에는 정중함 단계가 하나뿐이고, 두 번째 단계를 나타낼 `alert`와 `status` role은 live region에 함께 달 수 없습니다. |
...
| hover에서 타이머 일시정지·재개 | 포인터, 손가락, focus, 앱이 모두 돌아오면 시계가 처음부터 | 방금 다 읽은 독자에게는 남은 2초가 아니라 온전한 수명을 다시 주는 편이 낫습니다. |
