...
SVG stroke에는 CSS 그러데이션을 줄 수 없어서, 링은 바의 fill을 이루는 바로 그 두 stop으로 같은 135°의 `<linearGradient>`를 스스로 만듭니다. 요소 하나를 더 쓸 값어치가 있습니다 — 훑고 지나가는 바 옆의 납작한 링은 하나의 아이디어에 재질이 둘인 것이니까요.
