...
세 재질은 "이 표면이 페이지에 대해 얼마나 자기를 선언하는가"에 답하는데, **불려 나와야** 했던 팝업은 이미 답을 했습니다. 그리고 popover는 진짜로 떠 있습니다. elevation 사다리가 존재하는 바로 그 경우이므로, 평평하게 눕힐 수도 있는 선택지로 내놓는 대신 맨 위 칸에 고정되어 있습니다.
