...
- 판에 적힌 문구는 **트리거**가 자기 tooltip으로 나릅니다. 스크린 리더는 이 경로로 문구를 얻습니다. 판 자체는 semantics에서 제외되는데, 떠 있는 노드가 같은 문구를 되풀이하면 스크린 리더가 두 번 읽기 때문입니다. `content`가 `Text`이면 그 문자열이 알아서 쓰이고, 그 외에는 `semanticLabel`이 필요합니다.
- 감싸개는 레이아웃에 상자도, 자기 focus stop도 더하지 않습니다. 자식은 원래의 그것으로 남습니다.
- hover와 길게 누르기, focus에서 열리고, 셋이 각자 판을 붙잡습니다. 판은 모두 끝나야 사라지므로, 키보드로 연 것을 스쳐 가는 포인터가 닫지 않습니다. 포인터를 트리거에서 판 위로 옮겨도 닫히지 않고, <kbd>Escape</kbd>로 닫힙니다.
