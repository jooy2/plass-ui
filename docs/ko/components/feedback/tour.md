...
**`target`은 세 가지 형태이고, 먼저 쓸 것은 ref입니다.** **ref**는 컴파일러가 확인해 주고 이름을 바꿔도 살아남습니다. **선택자**는 누군가 클래스 이름을 바꾸는 순간 매칭이 멈추는 문자열이고, 그러면 투어는 빈 배경 위에 구멍을 뚫은 채로 계속 돕니다. 이 페이지가 렌더링하지 않은 것이 대상일 때 유일하게 동작하는 형태라서 남겨 두었습니다. **게터**는 질의 하나로 찾을 수 없는 대상에 씁니다.
...
투어는 계속 바뀌는 페이지 위를 돕니다. 아래쪽에서 무언가 로딩을 끝낼 수 있고, 이미지가 도착할 수 있고, 창 크기가 바뀔 수 있습니다. 그러면 빛은 빈 배경 위에 남습니다.
...
- `title`과 `content`는 polite live region이라, 포커스가 Next에 남은 채 다음 단계로 넘어가도 카드에 새로 적힌 내용을 읽어 줍니다.
...
- 투어가 열리면 카드가 포커스를 받으므로 <kbd>Escape</kbd>가 카드에 닿고, 투어가 닫히면 포커스를 원래 자리로 돌려줍니다.
- `dismissible`이 `false`가 아니면 <kbd>Escape</kbd>가 투어를 끝냅니다.
- 단계의 `title`은 live region이고, 제목이 없는 단계에서는 `content`가 그 역할을 합니다. 그래서 포커스가 Next에 남은 채 다음 단계로 넘어가도 카드에 새로 적힌 내용을 읽어 줍니다.
