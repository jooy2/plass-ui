...
- `label`은 모든 손잡이에 이름을 주고, 각 손잡이는 `aria-controls`로 앞 pane을 가리킵니다. pane에 자기 `id`가 있으면 그것을 씁니다. `label`이 없으면 손잡이가 둘인 분할은 구분선 둘과 숫자 둘로만 읽힙니다.
...
| `role="separator"`, `aria-valuenow` | 슬라이더 시맨틱 | Flutter의 트리에는 둘 다 없고, 값이 붙은 컨트롤이 정직한 서술입니다. |
