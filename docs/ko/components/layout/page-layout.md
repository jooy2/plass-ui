...
- children은 진짜 `<main>` 안에 들어가고, 그것이 `main` landmark입니다. 페이지당 정확히 하나여야 하며, 그걸 보장하는 것이 레이아웃입니다. 다른 레이아웃 안의 레이아웃은 children을 `<div>`에 넣고, 건너뛰기 링크도 `id`도 두지 않습니다.
...
- 콘텐츠는 `SemanticsRole.main`으로 감싸집니다. 반대쪽의 `<main>` 요소가 하는 것과 같은 주장입니다: 여기가 chrome이 아닌 부분이라는 것. 다른 레이아웃 안의 레이아웃은 이 role을, 그리고 `mainSemanticLabel`도 함께 바깥 레이아웃에 맡깁니다.
