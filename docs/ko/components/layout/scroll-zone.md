...
메커니즘은 **평범한 스크롤 컨테이너**이고, 이 컴포넌트가 제공하는 전부는 그것을 움직이는 방법입니다. 스와이프, 트랙패드의 두 손가락 드래그, 스크롤바는 전부 플랫폼 자신의 것이고 가로채지 않습니다. 그 위에 더한 것은 휠도 손가락도 없는 포인터를 위한 버튼 한 쌍과, 페이지를 넘기는 것이 아니라 잡아당기는 것으로 읽히는 띠를 위한 마우스 드래그, 그리고 가로 띠에게는 달리 쓸 데가 없는 세로 방향 휠입니다.
...
기본값 `auto`는 전부 들어맞을 때 둘 다 그리지 않습니다. 넘치지 않는 줄은 스크롤러가 아니기 때문입니다. 한 번 스크롤러가 되면 둘 다 그리고 갈 곳이 없는 쪽을 disabled로 두는데, 이는 `always`가 그리는 것과 똑같습니다. `auto`가 정하는 것은 이 띠에 스크롤 버튼이 있느냐이지, 지금 이 순간 둘 중 어느 쪽이 존재하느냐가 아닙니다. `always`는 첫 페인트부터 둘 다 그립니다. 내용이 나중에 도착하는 띠가 원하는 것이 그것입니다. `none`은 둘 다 그리지 않고 띠를 휠과 방향키와 드래그에 맡깁니다.
...
inline 버튼은 갈 곳이 없을 때도 자기 자리를 지킵니다. 자리가 나타났다 사라지면 방금 끝에 다다른 포인터 아래에서 띠의 크기가 바뀌기 때문입니다. 그리고 그 자리에는 보이지 않게 숨은 버튼이 아니라 **disabled 버튼이 그려집니다**. 자리 값은 어차피 치르고 있고, 띠 옆에 비워 둔 자리는 절제로 읽히지 않습니다. 상자 한쪽에만 이상하게 붙은 여백으로 읽힙니다. `overlay` 버튼은 반대의 경우여서 사라집니다. 지킬 자리가 없으니 없애는 데 드는 비용이 없습니다.
