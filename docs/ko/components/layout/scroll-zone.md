...
아무것도 변형되지 않습니다. `translate`로 옮기는 트랙이었다면 [하우스 규칙](../../design/design-language)의 예외를 요구해야 했겠지만 스크롤 오프셋은 그럴 필요가 없습니다. 그리고 그 덕분에 따로 말해 주지 않아도 RTL에서 띠가 반대 방향으로 흐르고, 스크롤바가 정직해집니다.
...
제스처의 세로 성분만 읽습니다. 트랙패드의 두 손가락과 틸트 휠은 이미 띠를 옆으로 스크롤하므로 그대로 둡니다. 세로 zone은 아예 건드리지 않습니다. 휠은 이미 그 방향으로 움직이기 때문입니다. 띠에 갈 곳이 없어진 다음의 이야기는 `overscroll`입니다.
...
마지막 카드까지 온 선반도 여전히 포인터 아래에 있는 그것입니다. 기본값 `contain`은 제스처를 거기에 붙잡아 둡니다. 띠가 다 떨어진 뒤에 휠이 한 칸 더 굴렀다고 해서 페이지가 움직이기 시작하지는 않습니다. 독자가 요청한 적도 없고 대개 예상하지도 못하는 도약이기 때문입니다.

`auto`는 끝에서 휠을 페이지에 돌려줍니다. 중첩된 스크롤러를 그냥 두었을 때의 동작입니다. 다만 조금 전까지 띠를 굴리던 제스처는 이때도 띠가 붙잡습니다. 페이지가 넘겨받는 시점은 독자가 잠깐 멈춘 뒤이지, 튕기던 도중이 아닙니다.

이 붙잡음이 덫이 되지 않게 하는 것은 두 가지입니다. 내용이 다 들어가는 띠는 스크롤러가 아니어서 어느 쪽이든 아무것도 붙잡지 않고, 붙잡는 축은 띠가 실제로 달리는 축뿐이라 가로 선반 위에서 손가락을 내리면 페이지는 그대로 스크롤됩니다.
...
나머지 한 축은 스크롤러 자신의 `overscroll-behavior`가 맡습니다. 선반 끝을 지나친 두 손가락 스와이프가 페이지를 뒤로 넘기는 것을 막는 것이 그쪽입니다.
...
갈 곳이 없어진 띠도 스크롤을 뒤에 있는 것에 넘기는 대신 그대로 가져갑니다. 브라우저에서 `contain`이 내리는 것과 같은 결정입니다.
...
- 스크롤 버튼은 진짜 이름이 붙은 진짜 버튼이고, 그 이름은 `previousLabel`과 `nextLabel`이 정합니다. 셰브런 하나가 든 원반에는 접근 가능한 이름이 전혀 없고, 그것이 [`PlIconButton`](../inputs/icon-button)의 `label`이 불가능하게 만들려는 결함입니다.
...
- 스크롤할 곳이 남아 있는 동안 띠 자체가 tab 대상이 되므로, `buttons: none`이거나 안에 포커스 가능한 것이 없는 띠도 키보드로 움직일 수 있습니다. 띠 방향의 방향키는 읽는 방향을 따라 한 줄씩, <kbd>Page Up</kbd>과 <kbd>Page Down</kbd>은 한 화면씩, <kbd>Home</kbd>과 <kbd>End</kbd>는 양 끝까지 옮깁니다. 포커스 링은 띠 둘레에 `color`로 그려집니다.
...
| — | `controller` | Flutter는 `ScrollController`로 스크롤 뷰를 움직이고, 오프셋이 필요한 호출자에게는 그것이 붙은 객체를 건네야 합니다. |
