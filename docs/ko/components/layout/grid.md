...
`alignItems`는 한 줄 안에서 칸들이 서로에 대해 어떻게 놓이는가이고, 기본값은 `stretch`입니다. 아무도 시키지 않았는데 카드 한 줄의 높이가 맞는 이유가 그것입니다. `alignSelf`는 그것을 칸 하나에서만 덮어씁니다. `alignContent`는 그리드가 담긴 상자보다 짧을 때 줄들이 어디에 놓이는가이고, 자기 높이가 붙은 그리드에서만 보입니다.
...
`PlGrid`는 `Column` 안에 줄마다 run 하나를 만들고, 너비를 비교하는 대신 **칸 수**를 세어 줄로 묶습니다. 모든 칸이 정수 개의 칸이라서, 칸을 세는 것은 `double` 둘을 비교할 때처럼 반올림된 픽셀 하나로 스스로와 어긋날 수가 없습니다.
...
한 줄의 높이는 가장 높은 칸의 높이입니다. 모든 칸을 먼저 자기 높이로 배치한 뒤, stretch인 칸은 줄의 높이로 한 번 더 배치하고 나머지 칸은 그 높이의 시작, 가운데, 끝에 놓습니다. 어느 칸에도 intrinsic 높이를 묻지 않으므로, `LayoutBuilder`로 자기 크기를 재는 위젯도 칸에 넣을 수 있습니다. 다른 `PlGrid`, 차트, 표, 슬라이더가 그렇습니다.

첫 배치에서는 칸이 채울 높이가 없어서, 칸 바로 안의 `Column`에 둔 `Spacer`나 `Expanded`는 차지할 공간이 없습니다. stretch인 칸의 아래쪽에 footer를 붙이려면 그 `Column`에 `MainAxisAlignment.spaceBetween`을 주세요.
