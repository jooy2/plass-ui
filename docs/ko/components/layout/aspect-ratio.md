...
`ratio`는 `double`이고 `16 / 9`처럼 나눗셈으로 씁니다. Flutter가 다른 곳에서도 종횡비를 지정하는 방식입니다. 그리고 값을 자르지 않고 단언합니다. 0인 비율은 모양이 아니라 실수입니다.
...
비율 위에 얹힌 단 하나의 편의입니다. 다섯 단어는 `object-fit`의 것 그대로입니다. `cover`는 상자를 채우고 넘치는 것을 잘라 내고, `contain`은 레터박스를 만들며, `fill`은 늘이고, `none`은 제 크기 그대로 그리고, `scale-down`은 상자보다 작은 것을 키우지 않는 `contain`입니다.
...
| `ratio`가 `'16 / 9'` | `ratio`가 `16 / 9` | 나눗셈으로 쓰는 `double`이고, Flutter가 다른 곳에서도 종횡비를 지정하는 방식입니다. 파싱할 문자열 형태가 없습니다. |
