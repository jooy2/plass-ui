...
목록은 맞는 명령 가운데 처음 100개를 그립니다. 입력할수록 목록이 좁혀지므로 더 아래에 있는 명령도 한두 글자면 닿습니다. 맞은 명령이 그린 것보다 많은 동안에는 목록 아래 한 줄이 몇 개가 더 있는지 알려 줍니다. "150 more"처럼, 레이블 팩의 `chartMore`로 적힙니다.
