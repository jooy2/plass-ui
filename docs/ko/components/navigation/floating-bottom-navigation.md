...
<p class="plass-lede">창의 아래 가장자리에서 떠 있는 둥근 목적지 한 줄입니다. 맑은 유리 캡슐 안에 색이 든 유리 키 하나가 올라타 있는 모양이고, 여기에 더한 것은 없습니다.</p>
...
- 모든 원반은 순서대로 놓인 각자의 포커스 정지입니다.
- 원반의 이름은 항목의 <Fw react="children" flutter="label" code />이고, 화면에는 그려지지 않습니다.
...
- 현재 목적지는 **selected**로 표시됩니다. Flutter에서 `aria-current`에 가장 가까운 말이자, 원반을 토글로 알리지 않는 말입니다.
- 각 원반은 이름과 탭 동작을 든 버튼 노드이고, 안의 글리프는 제외됩니다. 그래서 글리프가 읽을 것을 하나 더 만들지 않습니다.
