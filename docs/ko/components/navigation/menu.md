...
<p class="plass-lede">무언가를 눌렀을 때 나타나는 동작 목록입니다. roving 포커스와 타이프어헤드, 안전 삼각형이 딸린 서브메뉴, 그리고 그 모두를 스크린 리더에게 뜻이 통하게 하는 role까지 들어 있습니다.</p>
...
`variant`은 없습니다. `PlModal`에 없는 것과 같은 이유입니다. 세 재질은 "이 표면이 페이지에 대해 얼마나 자기를 선언하는가"에 답하는데, 포인터를 가져간 팝업은 이미 그 질문에 답했습니다. `elevation`도 없습니다. 메뉴는 진짜로 떠 있고, 그것이 사다리가 존재하는 유일한 경우라서 맨 윗단에 고정되어 있습니다. 라이브러리 전체에서 공유 축이 뜻하는 바는 [prop 규칙](../../design/prop-conventions)에 있습니다.
...
서브메뉴를 여는 행은 다른 모든 항목과 같은 행이고, 셰브런을 하나 달고 있을 뿐입니다. 호버로, <kbd>Enter</kbd>로, 그리고 그쪽을 가리키는 화살표 키로 열립니다. 대각선으로 움직여도 닫히지 않습니다. Base UI가 포인터에서 팝업까지의 안전 삼각형을 추적합니다.
...
- 메뉴가 닫히면 키로 열었든 포인터로 열었든 포커스는 트리거로 돌아갑니다. 행의 핸들러가 포커스를 다른 곳으로 보냈다면 거기에 둡니다. `PlButton` 트리거는 Flutter의 focus highlight mode가 키보드의 것인 `traditional`일 때만 링을 그리므로, 터치 화면에서 탭한 뒤에는 링이 남지 않습니다.
...
- 비활성 행도 목록에 남지만, 화살표 키와 타이프어헤드는 그 행을 건너뜁니다. 쓸 수 없을 때 사라지는 행은 길이가 바뀌는 메뉴입니다.
