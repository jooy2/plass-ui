...
<p class="plass-lede">누군가 요청할 때까지 덮여 있는 내용입니다. 결말이나 정답, 아직 아무도 보겠다고 하지 않은 사진에 씁니다.</p>
...
덮개는 **블러**이고, 그 블러가 설계의 전부입니다. 독자는 거기 무언가가 있다는 것과 분량이 대략 얼마인지, `maxHeight`가 있다면 잘려 있다는 것까지 볼 수 있습니다. 볼 수 없는 것은 내용뿐이고, 스포일러가 막으려는 것도 그 하나입니다.
...
세 재질을 *컨테이너*의 방식으로 씁니다. 시트에는 색이 들어가지 않습니다. 스포일러가 담는 것은 사진이고 문단이고 결말이며, 그것들은 자기 색을 가지고 옵니다. 색 계열은 버튼과 얇은 선까지만 닿고 멈춥니다.
...
비워 두면 상자는 담고 있는 것만큼 정확히 높습니다. 문단이나 사진에는 그것이 맞는 기본값입니다. 흐릿한 내용이 한 페이지를 채우면 아무것도 없는 페이지가 되어 버릴 만큼 긴 것에는 지정하세요.
...
- 누른 버튼은 더는 받을 수 없는 포커스를 붙들고 있지 않습니다. 드러내면 포커스가 내용으로 옮겨 가서 다음 Tab이 그 안의 첫 항목에 닿고, 숨기면 드러내기 버튼으로 돌아옵니다. 포커스가 페이지의 다른 곳에 있었다면 그 자리에 그대로 둡니다.
