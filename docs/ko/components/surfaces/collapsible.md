...
<p class="plass-lede">혼자 서 있는, 접히는 섹션 하나입니다. <code>PlAccordion</code>이 묶음으로 가지고 있는 바로 그 접힘을 옆에 아무것도 없이 떼어 놓은 것이라, 남의 목록 속 자리가 아니라 자기 <code>open</code>이 필요합니다.</p>
...
세 재질을 *컨테이너*의 방식으로 말합니다. 시트에는 색이 들어가지 않습니다. 접힘은 남의 내용을 담기 때문입니다. `ghost`는 흐르는 산문 속이나 카드 안에서 쓰는 것입니다 — 맨 "더 보기" 한 줄이 페이지에 사각형 하나를 빚질 이유는 없습니다.
...
헤더를 통째로 자기 컨트롤로 바꿉니다. 넘긴 요소가 **트리거가 됩니다**. 클릭 핸들러도, 열림 상태도, 패널을 가리키는 포인터도 전부 그것에 건네지므로 따로 연결할 것이 없습니다.
...
- `keepMounted`는 그냥 남겨 둡니다. 만드는 데 비용이 큰 내용이나, 접혀 있는 동안에도 살아 있어야 하는 폼 상태를 가진 내용을 위한 것입니다.
...
- 헤더는 패널이 열려 있는지를 알리고 자기가 여는 패널을 가리키는 진짜 `<button>`입니다. 그 연결은 Base UI의 것입니다.
- `action`은 트리거 바깥에 있으므로 버튼 안에 중첩된 요소가 아니라 자기 focus stop을 가집니다. 중첩했다면 브라우저가 파싱하면서 다시 썼을 것입니다.
- 비활성 접힘의 트리거는 비활성 버튼입니다. 탭 순서에서 빠지고, 패널은 있던 그대로 남습니다.
