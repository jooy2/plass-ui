...
기본은 꺼져 있고, 섹션은 여백으로 구분됩니다. 켜면 `--plass-divider` 헤어라인으로 나뉩니다. `PlList`와 `PlTable`에 선을 긋는 것과 같은 중립 잉크라서 라이브러리의 모든 내부 선이 한 종류가 됩니다. 시트 자신의 흰 테두리는 쓰지 않습니다. 그 선은 뒤에 페이지 배경이 있어야 읽히는데, 판 한가운데를 가로지르면 뒤에 있는 것은 판이기 때문입니다. 선이 양 끝까지 닿아야 하므로 여백은 카드에서 각 섹션으로 옮겨 갑니다.
