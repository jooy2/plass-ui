...
`preview.url`은 `http`, `https`, `mailto`이거나 상대 주소일 때 카드의 `href`가 됩니다. 메시지는 대개 읽는 사람이 아닌 다른 사람이 쓴 것이라, 그 밖의 scheme을 가진 주소는 같은 카드로 그려지되 누를 것이 없습니다.
...
- 링크 카드는 `preview.url`이 버블이 따라갈 주소일 때 진짜 `<a>`가 되고, `newTab`은 새 페이지가 `window.opener`로 되돌아오지 못하게 하는 `rel`을 함께 붙입니다. 그 밖의 주소에서는 링크 없이 카드만 그려지므로, 목적지가 아닌 것을 목적지라고 읽어 주지 않습니다.
