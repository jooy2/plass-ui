...
- 프레임 위쪽 모서리의 버튼으로 멈추고 다시 시작합니다. 키보드가 가장 먼저 닿는 곳이고, 이름이 누르면 무엇을 하는지 말합니다. 넘어가는 동안은 "Stop slide show", 멈춘 뒤에는 "Start slide show"입니다.
- 포인터가 위에 있는 동안 **잠시 멈추고**, 포인터가 떠나면 이어서 넘어갑니다.
- 손가락이 띠에 닿아 있는 동안에도 잠시 멈춥니다. 띠를 끌고 있지 않아도 그렇습니다. 손가락을 떼면 띠가 자리 잡은 슬라이드에 `interval` 한 번을 온전히 머문 뒤에 넘어갑니다.
- 배경에 있는 동안에도 잠시 멈춥니다. 브라우저 탭이 보이지 않을 때나 앱이 독자 앞에 있지 않을 때가 그렇습니다. 다시 보이면 떠날 때의 슬라이드에 `interval` 한 번을 온전히 머문 뒤에 넘어갑니다.
- 마운트된 채로 숨겨진 동안에도 잠시 멈춥니다. 다시 보이면 숨겨질 때의 슬라이드에 `interval` 한 번을 온전히 머문 뒤에 넘어갑니다.
- 포커스가 들어오면 **멈춥니다**. 이쪽이 중요합니다. 슬라이드로 탭해 들어온 키보드 독자는 그것을 읽고 있는 중입니다. 그 뒤로는 포커스나 포인터가 떠났는지와 상관없이 버튼으로 다시 시작할 때까지 멈춰 있습니다. 포커스가 들어와도 멈추지 않는 자리는 그 버튼 하나뿐입니다.
- 화살표나 점을 눌러도 멈춥니다. 누를 때 포커스가 옮겨지는지와는 상관없습니다. 버튼으로 다시 시작한 뒤에는 화살표와 점을 눌러도 멈추지 않습니다.
- 움직임을 줄여 달라고 한 독자에게는 멈춘 채로 시작하고, 버튼으로 시작할 수 있습니다.

::: fw react

- 숨겨졌다는 것은 트랙에 너비가 없다는 뜻입니다. 마운트된 채로 닫힌 `PlAccordion`이나 선택되지 않은 `PlTab` 안이 그렇습니다.
- 현재 슬라이드를 알리는 live region이 넘어가는 동안 **침묵합니다**. 5초마다 새 슬라이드 이름을 말하는 스크린 리더가 그 페이지를 못 쓰게 만드는 것이기 때문입니다. 멈춘 뒤에는 독자가 어디로 갔는지 말합니다.
- uncontrolled 캐러셀은 `onValueChange`가 없어도 넘어갑니다. controlled 캐러셀은 새 index가 `value`로 돌아와야 움직입니다.
...
- 앱이 배경에 있다고 보는 것은 lifecycle 상태가 `hidden`, `paused`, `detached`일 때뿐입니다. 보이기는 하지만 포커스를 잃은 창은 `inactive`이고, 포커스를 잃은 브라우저 창에서처럼 캐러셀은 거기서 계속 넘어갑니다.
...
점 하나하나는 자기가 가는 슬라이드의 이름이 붙은 진짜 버튼입니다. 이 줄은 읽어 주는 표시가 아니라 이동하는 수단입니다. 버튼은 가로세로 24px의 누름 영역이고 점은 그 가운데에 그려집니다. 영역끼리 나란히 붙어 있어서, 몇 픽셀짜리 점이라도 휴대폰에서 쉽게 누를 수 있습니다.
...
- `autoPlay`를 켜면 멈추고 다시 시작하는 버튼이 포커스 순서의 맨 앞에 옵니다. 눌림 상태를 두는 대신 하는 일에 따라 이름이 바뀌고, 두 이름은 `playLabel`과 `stopLabel`이 정합니다.
...
- 독자가 어디 있는지는 polite live region에서 문장으로 안내됩니다. 캐러셀이 스스로 넘어가는 동안에는 말하지 않습니다.
