...
<p class="plass-lede">실시간으로 바뀌는 정보를 조금 담고 떠 있는 알약입니다. 돌아가고 있는 녹화, 올라가고 있는 업로드, 아직 읽지 않은 알림 두 개 같은 것을 올립니다.</p>
...
세 재질을 **컨트롤**의 방식으로 씁니다. 표면이 색을 받습니다. [`PlButton`](../inputs/button)이나 [`PlChip`](../display/chip)과 같습니다. 알약은 남의 내용을 담은 시트가 아니라, 색이 입혀지는 그 물건 자체이기 때문입니다.
...
- 누를 것이 없는 알약은 컨트롤이 아니고 아무 role도 선언하지 않습니다. 핸들러를 주면 가운데가 진짜 버튼이 되어 키보드로 닿을 수 있고 그것으로 안내됩니다. `details`가 있으면 그 버튼은 패널이 열려 있는지도 알립니다.
- `endIcon`은 그 버튼 바깥에 있으므로 거기 놓인 컨트롤은 자기 focus stop이 있습니다.
