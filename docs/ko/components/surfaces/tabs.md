...
그래서 스크롤 중이라는 것을 스스로 말해야 하는데, 스크롤바는 그 일을 하지 못합니다. macOS에서는 띠가 움직이는 동안에만 나타나는 오버레이이고 — 독자가 더 볼 것이 있는지 판단하는 순간은 전부 그 나머지 시간입니다 — Windows에서는 라벨 줄 아래에 늘 자리를 차지하는 15픽셀짜리 가구입니다. 둘 다 감추고, 대신 아직 탭이 남아 있는 쪽 끝을 흐립니다. 남아 있는 쪽만입니다. 갈 곳 없는 끝이 흐려진 바는 거짓말하는 바입니다.
