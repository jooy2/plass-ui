...
- `prefers-reduced-motion`에서는 띠가 멈추고 첫 번째 사본만 그려집니다. 상자는 띠를 잘라 내지 않고 띠를 따라 스크롤되므로, 띠가 여러 줄로 접히지 않고 한 줄로 남습니다. 상자 밖에 있던 것은 스크롤하거나, 항목이 focus를 받으면 Tab으로, 또는 상자에서 화살표 키로 닿습니다. 상자는 스크롤할 것이 있는 동안 Tab 정지점이 됩니다. 그 정지점에 이름을 주려면 `role="group"`과 `aria-label`을 넘기세요.
- **첫 번째 사본만 읽히고 Tab으로 닿습니다.** 나머지는 `aria-hidden`을 답니다. 아니면 스크린리더가 띠 위의 모든 것을 깔린 횟수만큼 읽습니다. `inert`도 답니다. `aria-hidden`은 focus를 막지 않아서, 링크 열 개짜리 띠가 Tab 정지점 서른 개가 되기 때문입니다.
...
- 플랫폼에서 애니메이션이 꺼져 있으면(`MediaQuery.disableAnimations`) 띠는 시작한 자리에 서 있고 첫 번째 사본만 깔립니다. 상자는 띠를 잘라 내지 않고 띠를 따라 스크롤되므로, 띠의 높이도 레이아웃도 그대로입니다. 상자 밖에 있던 것은 스크롤하거나, 항목이 focus를 받으면 Tab으로, 또는 상자에서 화살표 키로 닿습니다. 상자는 스크롤할 것이 있는 동안 Tab 정지점이 됩니다.
...
| React | Flutter | 이유 |
| --- | --- | --- |
| 첫 사본 이후에 붙는 `aria-hidden`과 `inert` | `ExcludeSemantics`와 `ExcludeFocus` | 같은 두 가지 제외에 대한 프레임워크 자신의 이름입니다. |
