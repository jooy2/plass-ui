...
- `prefers-reduced-motion`에서는 띠가 멈추고 첫 번째 사본만 그려집니다. 상자는 띠를 잘라 내지 않고 띠를 따라 스크롤되므로, 띠가 여러 줄로 접히지 않고 한 줄로 남습니다. 상자 밖에 있던 것은 스크롤하거나, 항목이 focus를 받으면 Tab으로, 또는 상자에서 화살표 키로 닿습니다. 상자는 스크롤할 것이 있는 동안 Tab 정지점이 됩니다. 그 정지점의 이름은 `label`로 줍니다. 상자는 그 이름을 단 `role="group"`이 되고, 띠가 흐르는 동안에도 그대로입니다.
