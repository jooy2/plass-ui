...
- 여기의 모든 효과가 그렇듯 `transform` 축약형이 아니라 독립 속성인 `translate`로 움직입니다. 그래서 호출자가 같은 요소에 직접 걸어 둔 transform이 살아남습니다.
...
- **멈출 방법을 주십시오.** 이 효과는 끝나지 않습니다. 다른 내용 옆에서 5초 넘게 움직인다면 페이지에 그것을 멈추는 컨트롤이 있어야 하고, [PlAnimateMarquee 예제](./animate-marquee#paused)처럼 `paused`에 연결한 버튼이면 됩니다. [WCAG 2.2.2](https://www.w3.org/WAI/WCAG22/Understanding/pause-stop-hide.html)가 이것을 요구합니다. 동작을 줄여 달라는 시스템 설정은 이 컨트롤을 대신하지 못합니다. 읽는 사람이 직접 찾아서 켜야 하기 때문입니다.
