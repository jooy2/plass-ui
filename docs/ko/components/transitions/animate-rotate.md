...
- `prefers-reduced-motion`에서는 아무것도 돌지 않습니다. 효과가 원래 시작했을 시점까지는(`delay` 포함) 내용이 `to`에 서 있고, 그 시점부터 마지막 프레임을 보여 줍니다. 도착이라면 `to`, `mode="out"`인 회전이라면 `from`이고, `fade`를 끄지 않았다면 흐려져 사라집니다. `animationend`도 그대로 발생합니다. 도착에는 맞는 처리이고, 회전에는 한 번 생각해 볼 문제입니다. 도는 것 자체가 '무언가 진행 중'이라는 뜻이라면 [PlProgressCircular](../feedback/progress-circular)를 쓰세요. 그쪽은 멈추는 대신 느려집니다.
...
- 플랫폼에서 애니메이션이 꺼져 있으면(`MediaQuery.disableAnimations`) 아무것도 돌지 않습니다. 효과가 원래 시작했을 시점까지는(`delay` 포함) 내용이 `to`에 서 있고, 그 시점부터 마지막 프레임을 보여 줍니다. 도착이라면 `to`, `PlassAnimateMode.exit`인 회전이라면 `from`이고, `fade`를 끄지 않았다면 흐려져 사라집니다. 도착에는 맞는 처리이고, 회전에는 한 번 생각해 볼 문제입니다. 도는 것 자체가 '무언가 진행 중'이라는 뜻이라면 [PlProgressCircular](../feedback/progress-circular)를 쓰세요. 그쪽은 멈추는 대신 느려집니다.
