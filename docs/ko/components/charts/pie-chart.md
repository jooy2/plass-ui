...
- 그림은 **tab 정거장**입니다. <kbd>←</kbd>와 <kbd>→</kbd>로 그려진 조각을 하나씩 옮겨 다닙니다. 마지막 조각 다음은 첫 조각이고, 첫 조각 앞은 마지막 조각입니다. <kbd>Escape</kbd>로 읽던 값을 지우는데, 읽고 있는 조각이 있을 때만 차트가 이 키를 가져갑니다. 그 밖에는 차트를 담은 쪽으로 넘어가므로, 차트를 품은 sheet도 그대로 닫힙니다. 닿는 조각마다 이름과 값이 live region으로 읽힙니다.
