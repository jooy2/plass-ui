...
차트가 점의 `x`를 적는 자리에서는 x축에 <Fw react="tickFormat" flutter="format" code />이 있으면 그것을 거치고, 없으면 축약형으로 적습니다. 그래서 `12345`는 `12.3K`가 됩니다. 날짜는 날짜로 적습니다. 차트의 `format`은 `y`의 것이라 `x`에는 쓰이지 않습니다.
...
- 그림은 tab 정거장이고, 화살표 키가 **데이터를 준 순서대로** 점을 옮겨 다닙니다. 그리는 순서인 큰 것부터는 읽는 사람이 예상할 수 없는 순서입니다. 닿는 점마다 카드에 적힌 대로 live region으로 읽힙니다. `x`, series, `y` 순서이고, 버블이면 `y` 뒤 괄호 안에 `z`가 붙습니다.
- React에서는 차트 아래 표가 다른 차트가 쓰는 격자가 아니라 점마다 한 행입니다. 둘 다 자기 series의 다섯 번째인 두 점은 서로 아무 관계가 없고, 한 행에 묶으면 없는 관계를 만들어 내기 때문입니다. 열 이름은 축 label에서 따오고, 없으면 `x`, `y`, `z`입니다.
