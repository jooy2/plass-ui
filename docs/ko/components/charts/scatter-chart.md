...
점의 `z`는 반지름이 아니라 **넓이**입니다. 반지름으로 담으면 두 배인 값이 네 배 크기로 그려지고, 제곱근이 종이에 칠해진 잉크를 그 뒤의 숫자에 비례하게 만듭니다. 척도는 차트 전체에 하나이고 숨긴 series까지 포함해 잽니다. 그래서 같은 크기의 두 버블은 어디에 있든 같은 숫자를 뜻하고, series를 끄더라도 나머지 크기가 바뀌지 않습니다.
...
`color`를 직접 준 series는 이 한도에 세지 않습니다. 한도가 던지는 질문에 caller가 이미 답했으니까요.
...
- React에서는 같은 숫자가 차트 아래 표에도 적힙니다. 다른 차트가 쓰는 격자가 아니라 점마다 한 행입니다. 둘 다 자기 series의 다섯 번째인 두 점은 서로 아무 관계가 없고, 한 행에 묶으면 없는 관계를 만들어 내니까요. 열 이름은 축 label에서 따오고, 없으면 `x`, `y`, `z`입니다.
