...
**sparkline은 자기 범위에 맞춰 스스로 늘어나므로 띠가 언제나 꽉 찹니다.** 20px 높이에서도 읽히는 이유이자 함정입니다. 나란히 놓인 둘은 서로 다른 척도로 그려지므로, 가파르게 오르는 띠가 더 작은 숫자일 수 있습니다. 한 줄로 늘어놓을 때 같은 `min`과 `max`를 주면 small multiples 차트가 됩니다. 둘을 벗어난 값은 띠 밖으로 그려지지 않고 띠의 가장자리에서 잘리므로, 상자 밖으로 나가는 것이 없습니다.
