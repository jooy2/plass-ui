...
두 축이 모두 category이고 묻는 것이 *어디*일 때 격자를 쓰세요. 어느 요일의 몇 시인지, 어느 코호트의 몇째 주인지. 같은 데이터의 [막대 차트](./bar-chart)는 아무도 훑을 수 없는 막대 마흔 개가 됩니다.
...
배치는 잘라 나누기가 아니라 **정사각형화**입니다. 잘라 나눈 treemap은 값이 스무 개쯤 되면 폭 1px짜리 조각으로 끝나고, 그런 조각의 *넓이*는 아무리 정확해도 읽히지 않습니다. 읽는 사람은 대신 길이를 비교하는데, 그것은 담기로 한 양이 아닙니다.
...
- 그림은 차트의 이름을 지니고, 색이 말하는 것을 글로 넘깁니다.
- 그림은 **tab 정거장**입니다. <kbd>←</kbd>와 <kbd>→</kbd>로 칸을 하나씩 옮겨 다닙니다. 격자는 한 행씩 차례로, treemap은 큰 타일부터 돌고, 첫 칸과 마지막 칸에서 멈춥니다. 격자에서 <kbd>↑</kbd>와 <kbd>↓</kbd>는 같은 열의 위 행이나 아래 행으로 옮겨 가고, 빈 칸은 건너뜁니다. treemap에서는 <kbd>←</kbd>, <kbd>→</kbd>와 똑같이 움직입니다. <kbd>Escape</kbd>로 읽던 값을 지우는데, 읽고 있는 칸이 있을 때만 차트가 이 키를 가져갑니다. 그 밖에는 차트를 담은 쪽으로 넘어가므로, 차트를 품은 sheet도 그대로 닫힙니다. 닿는 칸마다 두 이름과 값이 live region으로 읽힙니다. tooltip을 끄면 키는 아무것도 하지 않고 읽히는 것도 없습니다.
- React에서 그림은 `role="img"`이고, 포커스가 가면 이름에 이어 행마다 한 줄씩 읽힙니다. 그 행의 칸들이 덮는 범위로, "Morning 1–5" 같은 모양입니다. 칸 자체는 차트 아래 표에 두 벌의 이름과 함께 적힙니다. 행은 옆에, 열은 위에 옵니다. 표는 그림의 설명이 아니라 형제입니다. 설명으로 두면 포커스할 때마다 모든 칸을 먼저 듣게 되기 때문입니다. Flutter에서는 넘기는 글에 모든 칸이 들어갑니다. 행마다 칸들이 이름과 값의 쌍으로 나열되고, 보낼 표가 없기 때문입니다.
