...
- React에서 그림은 `role="img"`이자 tab 정거장입니다. <kbd>←</kbd>와 <kbd>→</kbd>로 칸을 하나씩 옮겨 다니고, 격자에서는 <kbd>↑</kbd>와 <kbd>↓</kbd>로 같은 열의 위 행이나 아래 행으로 옮겨 갑니다. <kbd>Escape</kbd>로 읽던 값을 지우고, 닿는 칸마다 live region으로 읽힙니다.
