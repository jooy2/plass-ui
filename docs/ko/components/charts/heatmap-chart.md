...
- React에서 그림은 `role="img"`이자 tab 정거장입니다. <kbd>←</kbd>와 <kbd>→</kbd>로 칸을 하나씩 옮겨 다니고, 격자에서는 <kbd>↑</kbd>와 <kbd>↓</kbd>로 같은 열의 위 행이나 아래 행으로 옮겨 갑니다. <kbd>Escape</kbd>로 읽던 값을 지우는데, 읽고 있는 칸이 있을 때만 차트가 이 키를 가져갑니다. 그 밖에는 차트를 담은 쪽으로 넘어가므로, 차트를 품은 sheet도 그대로 닫힙니다. 닿는 칸마다 live region으로 읽힙니다.
- React에서 그림에 포커스가 가면 이름에 이어 행마다 한 줄씩 읽힙니다. 그 행의 칸들이 덮는 범위로, "Morning 1–5" 같은 모양입니다. 칸 자체는 차트 아래 표에 두 벌의 이름과 함께 적힙니다. 행은 옆에, 열은 위에 옵니다. 표는 그림의 설명이 아니라 형제입니다. 설명으로 두면 포커스할 때마다 모든 칸을 먼저 듣게 되기 때문입니다. Flutter에서는 넘기는 글에 모든 칸이 들어갑니다. 행마다 칸들이 이름과 값의 쌍으로 나열되고, 보낼 표가 없기 때문입니다.
