...
간격은 크기 사다리를 따라 늘어나지 않고 고정입니다. 그래야 `sm` 차트와 `lg` 차트에서 점선이 같은 리듬으로 읽힙니다. 점선으로 만들 선이 있어야 뜻이 있으므로 막대에서는 아무 일도 하지 않고, 쌓인 띠에서도 마찬가지입니다. 띠는 채움이 곧 표시라 위쪽에 그어진 선이 없습니다. 선이 점선으로 그려지면 legend 항목도 사각형 대신 짧은 점선으로 그려져, 범례가 plot과 같은 말을 합니다. 그래도 `dashed`에는 그 선이 무엇인지 말하는 이름을 함께 붙이세요.
