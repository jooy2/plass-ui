...
<p class="plass-lede">element 하나에 속성 하나면 끝납니다. 모든 컴포넌트가 논리 속성으로 배치돼 있으므로 dir="rtl"이 전부입니다. 설정할 것도, 방향을 두 번 적을 일도 없습니다.</p>
...
| 읽는 방향을 가리키는 chevron, breadcrumb, pagination 스테퍼, 서브메뉴, navigation rail, `PlTree`의 접힌 삼각형, `PlTransfer`의 화살표 | **뒤집힙니다.** 글리프 하나를 돌립니다 |
