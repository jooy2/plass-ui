...
병합은 키 단위입니다. 네 단어만 정한 프로바이더는 나머지 백세 개를 영어로 두고, 다른 프로바이더 안에 들어간 프로바이더는 자기가 이름 붙인 것만 바꾸고 나머지는 물려받습니다.
