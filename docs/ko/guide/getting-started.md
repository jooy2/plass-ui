...
반대 방향은 일부러 막아 뒀습니다. 여러분이 쓴 `p { margin: 1rem }`은 컴포넌트 안의 문단을 벌리지 않습니다. 컴포넌트의 `m-0`은 클래스고 여러분의 규칙은 type selector이기 때문입니다.
...
컴포넌트를 몇 개만 쓰는 프로젝트라면 통짜 import보다 gzip 기준 약 5 kB 작습니다. 그러면서도 Tailwind pass는 여전히 **한 번**이라 utility가 Tailwind 자신의 순서대로 나옵니다. 130개의 컴파일된 스타일시트가 아니라 더 좁은 스캔으로 배포하는 이유가 이것입니다. 컴파일된 파일을 이어 붙이면 공유 utility가 전부 컴포넌트 전용 utility 앞에 놓이는데, 충돌하는 두 utility 중 어느 쪽이 이기는지는 Tailwind의 정렬이 정합니다.
