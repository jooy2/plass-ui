...
<p class="plass-lede">사용자가 플랫폼에 움직임을 줄여 달라고 해 두었는지. 라이브러리는 움직이는 모든 곳에서 이미 이것에 답하고 있습니다. 이 hook은 애플리케이션이 직접 쓴 움직임을 위한 같은 답입니다.</p>
...
이 hook에 손을 뻗기 전에 답할 질문은 "이 효과가 셋 중 무엇인가"입니다. 움직임이 메시지를 나르고 있다면, 움직임을 끄는 대신 메시지를 움직임 밖으로 꺼내세요.
