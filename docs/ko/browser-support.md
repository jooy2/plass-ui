...
| `subgrid` | 117 | 71 | 16 | `caption="below"`인 masonry [`PlGallery`](./components/display/gallery). 캡션에 따로 행을 줍니다 | 캡션까지 넣어 행의 크기를 정하므로 사진이 `ratio`보다 길게 그려지고, 늘어난 높이를 어떻게 채울지는 `fit`이 정합니다 |
