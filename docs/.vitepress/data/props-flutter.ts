...
export const flutterPropTables: Record<string, PropRow[]> = {
  ...
  PlChip: [
    ...
    from('PlChip', 'deleteLabel', {
      ...
      description: {
        ...
        en: 'The name of the delete affordance. Never drawn. Left out, it is the label pack’s `removeItem` handed the chip’s text, which can be read only when `child` is a `Text`; a chip holding another widget is named by `remove` alone and should be given a name that says which chip it removes. Given, it is the whole name'
      }
    }),
  ...
  PlDataTable: [
    ...
    from('PlDataTable', 'caption', {
      type: 'Widget?',
      description: {
        ko: '격자 위, 시트 안에 그려집니다. semanticLabel이 없으면 Text인 caption의 문구가 표의 이름이 되고, 한 번만 읽힙니다',
        en: 'Drawn above the grid, inside the sheet. Without a semanticLabel, a Text caption names the table and is read once, as that name'
      }
    }),
    ...
    from('PlDataTable', 'elevation', { type: 'int', default: '0' }),
    {
      ...
      type: 'String?',
      description: {
        ko: '표를 스크린 리더가 부를 이름. 비워 두면 Text인 caption의 문구가 이름이 되므로, 다른 위젯으로 짠 caption이나 그려진 것과 달라야 하는 이름에 씁니다. 격자가 maxHeight나 담긴 상자보다 높아 스크롤되는 동안에는 Tab 정지점이 되는데, 그 정지점도 같은 이름으로 읽힙니다',
        en: 'The name a screen reader gives the table. Left out, a Text caption is the name, so this is for a caption built of other widgets or a name that has to differ from what is drawn. A grid taller than maxHeight or its box is a tab stop while it scrolls, and the stop takes the same name'
      }
    },
    {
      name: 'selectAllLabel',
  ...
  PlCarousel: [
    ...
    from('PlCarousel', 'autoPlay', {
      type: 'bool',
      default: 'false',
      description: {
        ko: '스스로 넘어가고, 프레임 위에 멈춤·시작 버튼이 생깁니다. hover에서, 그리고 트리에 남은 채 숨겨진 동안에는 잠시 멈추고, 포커스가 들어오거나 화살표나 점을 누르면 버튼으로 다시 시작할 때까지 멈춥니다. reduced motion에서는 멈춘 채로 시작합니다. onChanged가 있어야 합니다',
        en: 'Advances on its own, with a button over the frame that stops and starts it. Pauses on hover and while it is hidden but still in the tree; stops once the focus comes in or an arrow or a dot is pressed, until the button starts it again. Starts stopped for reduced motion, and needs onChanged'
      }
    }),
    ...
    from('PlCarousel', 'playLabel', { type: 'String?', default: "'Start slide show'" }),
    from('PlCarousel', 'stopLabel', { type: 'String?', default: "'Stop slide show'" }),
