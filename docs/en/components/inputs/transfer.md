...
- A move hands the focus to the first row that arrived, in the list it went to, and scrolls that list to it. The arrow that was pressed is disabled by that move, and would otherwise have let the focus fall out of the component. When a controlled pair's owner refuses the rows, the focus goes to that list itself, which is a group named by its heading.
