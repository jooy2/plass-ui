...
- The highlight is one number rather than a hover state per row, which is what makes the pointer and the arrow keys light the same row. The pointer lights a row only by moving onto it, so a row the arrow keys scroll under a resting pointer leaves the highlight where the keys put it.
