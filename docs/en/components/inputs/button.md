...
- **The font.** Neither package sets one — a button inherits whatever its host uses. That means the previews on this page are drawn in the documentation site's UI font under React, and in Flutter web's own default under Flutter. It is the same button in two typefaces, not two buttons.
