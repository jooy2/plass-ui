...
All three are announced as unavailable, and only `disabled` also leaves the focus order. A `loading` button also says the `loading` word from [the label set](../../guide/locales) after its name, which is what tells it apart from `readOnly`.
