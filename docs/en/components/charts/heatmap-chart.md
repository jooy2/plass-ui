...
- The drawing carries the chart's name, and what the colours say is handed over as text.
- The picture is a **tab stop**. <kbd>←</kbd> and <kbd>→</kbd> walk the cells one at a time, row by row on a grid and largest first on a treemap, and stop at the first and the last. On a grid <kbd>↑</kbd> and <kbd>↓</kbd> move to the same column in the row above or below, stepping over a gap; on a treemap they walk it as <kbd>←</kbd> and <kbd>→</kbd> do. <kbd>Escape</kbd> clears the readout. The key is taken only while a cell is being read; otherwise it goes on to whatever the chart sits in, so a sheet around it still closes. Each cell is announced in a live region as it is reached, with both of its names and what it is worth. With the tooltip turned off the keys do nothing and nothing is announced.
