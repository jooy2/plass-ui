...
- The picture is a **tab stop**. <kbd>←</kbd> and <kbd>→</kbd> walk the slices that are drawn, one at a time: after the last comes the first, and before the first the last. <kbd>Escape</kbd> clears the readout. The key is taken only while a slice is being read; otherwise it goes on to whatever the chart sits in, so a sheet around it still closes. Each slice is announced in a live region as it is reached, by its name and what it is worth. With the tooltip turned off the keys do nothing and nothing is announced.
