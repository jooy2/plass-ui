...
On by default. Turn it off for content that should reach the edges — an image, a table, a list that draws its own rows — and give the box `overflow-hidden` so the content is cut by the sheet's own corners.
