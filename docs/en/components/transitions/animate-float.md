...
- **Give the reader a way to stop it.** It never finishes, and anything that moves for more than five seconds beside other content needs a control on the page that stops it: a button wired to `paused`, as in the [PlAnimateMarquee example](./animate-marquee#paused). [WCAG 2.2.2](https://www.w3.org/WAI/WCAG22/Understanding/pause-stop-hide.html) asks for one. Asking the system for less motion is no substitute, because that is a setting the reader has to find and turn on first.
