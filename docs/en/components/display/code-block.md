...
- The copy button changes its own label, so once the clipboard has answered, the block also announces `copiedLabel` or `copyFailedLabel` to a screen reader.
- The raw toggle carries a toggled state.
