...
- **`getRowKey` before anything else.** Everything the table remembers is remembered _by key_: the selection, the range anchor, the identity React and Flutter reconcile rows by. Without it a row is keyed by its position in `rows`, which holds through a sort, a search and a page, and does not hold once `rows` changes. A row added at the top gives every row below it a new key, and a selection made before that moves to different rows. With `manual` paging the position is counted from the first row of the first page, so a row ticked on one page does not light the row in the same place on the next, but a page the server sends again with a row added still moves every key below it.
- **Every `index` a callback is handed is the row's position in `rows`**, not its place on the screen. `getRowKey`, `render`, `onRowClick` and `isRowSelectable` (`rowKey`, `cell`, `onRowPressed` and `isRowSelectable` in Flutter) get the same number for a row wherever a sort, a search or a page has put it. With `manual` paging, `rows` is the page the server sent, so the count starts at the top of that page.
