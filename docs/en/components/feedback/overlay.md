...
Off by default, which is the other way round from `PlModal` and the one prop here worth reading twice. A modal asks a question and Escape is the universal "no"; an overlay says _wait_ rather than asking anything, and a save that can be dismissed by a stray click is a save the user will think finished.
