...
- An empty range (`max` at or below `min`) leaves the bar at nothing. It is a caller's mistake rather than a state, and a full bar would report something untrue.
