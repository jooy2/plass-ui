...
> **Both packages ship the same library** — the same hundred and twenty-five components, the same prop vocabulary, the same tokens. They version independently, so the npm and pub.dev numbers will not always agree; see [all components](../components/).
