...
For a project using a handful of components this is about 5 kB gzipped smaller than the blanket import. It is still **one** Tailwind pass, so the utilities come out in Tailwind's own order — which is why this is shipped as a narrower scan rather than as 68 pre-compiled stylesheets. Concatenating pre-compiled files would put every shared utility ahead of every component-specific one, and Tailwind's sort is what decides which of two conflicting utilities wins.
