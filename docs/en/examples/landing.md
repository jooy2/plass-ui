...
| Hero | `PlChip` `PlTypography` `PlButton` `PlAvatar` `PlRating` | The stacked avatars are a `flex -space-x-2` with a ring on each `PlAvatar`, a negative margin and a ring, and nothing about avatars |
