/**
 * How a `PlStepper` eases its lines and its titles as `active` moves, which
 ...
 * `src/standalone.css` the way `how-to-steps.test.tsx` does and records the
 * transitions that actually run. No duration is asserted, only that they run,
 * and that under reduced motion they do not.
 */
...
function stepper(active: number) {
  return (
    <PlStepper active={active} linear={false} data-testid="stepper">
      {[
        <PlStep key="account" label="Account" />,
        <PlStep key="verify" label="Verify" />,
        <PlStep key="profile" label="Profile" />
      ]}
    </PlStepper>
  );
}
...
/**
 * The line after the first step, which `active` moving on to the second takes
...
async function renderStepper() {
  const screen = await render(stepper(0));
  const first = screen.getByTestId('stepper').element().querySelector('ol > li') as HTMLElement;
  ...
  const titles = ['Account', 'Verify'].map(
...
describe('a stepper', () => {
  it('eases a step’s line and the titles as `active` moves past it', async () => {
    const { screen, connector, titles } = await renderStepper();
    const lined = recordRuns(connector);
    const inked = titles.map((title) => recordRuns(title));

    await screen.rerender(stepper(1));
